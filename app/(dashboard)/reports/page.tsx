...
import InteractiveReportViewer from '@/components/ai/InteractiveReportViewer';
import { useReportData } from '@/components/ai/useReportData';
...
export default function ReportsPage() {
  ...
  const finalizeReport = (type: ReportType, content: string) => {
    ...
    // Fetch chart data for the interactive viewer
  ...
  const generateReport = async (type: ReportType) => {
    ...
    try {
      ...
      while (true) {
        ...
        if (done) {
          // Stream ended — finalize with whatever was accumulated
          if (accumulated) {
  ...
  const handleHistorySelect = (entry: HistoryEntry) => {
    ...
    if (!reportData.kpi) reportData.fetchData();
  };
          ...
          <div className="flex flex-col gap-6">
            ...
            {viewerReport && (
              <section aria-label="AI 생성 리포트">
                ...
                <InteractiveReportViewer report={viewerReport} reportData={reportData} />
              </section>
            )}
